...
export function Hero() {
  ...
  return (
    <section ref={ref} className="relative min-h-[100svh] flex items-center pt-16 lg:pt-24 pb-8 lg:pb-16 overflow-hidden">
      {/* Aurora Background */}
      <div className="absolute inset-0 bg-[#fafafa]">
        {/* Animated gradient orbs */}
        <motion.div
          className="absolute w-[400px] lg:w-[800px] h-[400px] lg:h-[800px] rounded-full opacity-30 blur-3xl"
          style={{
        ...
        />
        <motion.div
          className="absolute w-[300px] lg:w-[600px] h-[300px] lg:h-[600px] rounded-full opacity-20 blur-3xl"
          style={{
      ...
      <div className="container mx-auto px-4 lg:px-6 relative z-10">
        {/* Mobile: Compact Hero - Everything Above Fold */}
        <div className="lg:hidden">
          ...
          >
            <div className="inline-flex items-center gap-2 backdrop-blur-xl bg-white/60 border border-white/40 px-3 py-1.5 rounded-full shadow-md text-xs">
              <span className="flex h-2 w-2 relative">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="font-medium text-[#222]">
                <strong className="bg-gradient-to-r from-[#5fdafb] to-[#a855f7] bg-clip-text text-transparent">3 Plätze</strong> für Q1 2025
              </span>
            </div>
          </motion.div>
          ...
          {/* Headline - Compact */}
          <motion.h1 
            className="text-[2.5rem] leading-[1.02] font-black text-black text-center mb-3 tracking-tight"
          ...
          {/* Subheadline - Short */}
          <motion.p 
            className="text-base text-[#555] text-center mb-4 px-2"
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 1 } : {}}
            transition={{ delay: 0.2 }}
          >
            Mehr Patienten durch <strong className="text-black">digitales Marketing</strong> speziell für Ärzte
          </motion.p>
          ...
          >
            {[
              { value: "+340%", label: "Anfragen" },
              { value: "150+", label: "Praxen" },
              { value: "100%", label: "Garantie" },
            ].map((stat) => (
              <div key={stat.label} className="text-center">
                <div className="text-xl font-black bg-gradient-to-r from-[#5fdafb] to-[#22d3ee] bg-clip-text text-transparent">
                  {stat.value}
                </div>
                <div className="text-[10px] text-[#888] font-medium">{stat.label}</div>
              </div>
            ))}
          </motion.div>
          ...
          >
            <a 
              href="#contact" 
              className="flex items-center justify-center gap-2 w-full py-3.5 bg-black text-white font-bold text-sm rounded-xl active:scale-[0.98] transition-transform"
            >
            ...
            <a 
              href="#cases" 
              className="flex items-center justify-center gap-2 w-full py-3 bg-white/70 backdrop-blur-sm border border-black/10 text-black font-bold text-sm rounded-xl active:scale-[0.98] transition-transform"
            >
          ...
          {/* Trust - Minimal */}
          <motion.div
            className="flex justify-center gap-3 text-[10px] text-[#888] mb-6"
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 1 } : {}}
          ...
          >
            ...
            <span className="flex items-center gap-1"><span className="text-emerald-500">✓</span> 24h Antwort</span>
          </motion.div>

          {/* Social Proof Card - Compact, Tappable Feel */}
          <motion.div
            className="backdrop-blur-xl bg-black/90 text-white p-4 rounded-2xl border border-white/10 shadow-lg"
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="flex -space-x-2">
                {[1,2,3].map((i) => (
                  <div 
                    key={i} 
                    className="w-7 h-7 rounded-full bg-gradient-to-br from-[#5fdafb] to-[#a855f7] border-2 border-black flex items-center justify-center text-white text-[10px] font-bold"
                  >
              ...
              <span className="text-xs text-white/60">150+ Praxen</span>
            </div>
            <p className="text-xs text-white/70 italic mb-2">
              "In 6 Monaten haben wir unsere Patientenanfragen verdreifacht."
            </p>
            <p className="text-xs font-bold bg-gradient-to-r from-[#5fdafb] to-[#a855f7] bg-clip-text text-transparent">
              — Dr. Thomas Weber, Wien
            </p>
          </motion.div>
        </div>
        ...
        <div className="hidden lg:block">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            >
              ...
              <h1 className="text-5xl lg:text-6xl xl:text-7xl font-black text-black leading-[1.02] mb-6 tracking-tight">
                <motion.span
                  initial={{ opacity: 0, y: 20 }}
                  animate={isInView ? { opacity: 1, y: 0 } : {}}
